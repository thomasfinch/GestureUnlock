...
NSArray* Resample(NSArray *points, int num) {
    ...
    for ( i=1; i<workingPoints.count; i++ ) {
        ...
        if ((D + d) >= I) {
            ...
            newVal = [NSValue valueWithPoint:newPoint];
...
NSArray* Scale(NSArray *points, int resolution, float threshold) {
    ...
    for (int i = 0; i < [points count]; i++) {
        ...
        [scaled addObject:[NSValue valueWithPoint:CGPointMake(qx, qy)]];
    }
...
NSArray* TranslateToOrigin(NSArray *points) {
    ...
    for (int i = 0; i < [points count]; i++) {
        ...
        [translated addObject:[NSValue valueWithPoint:CGPointMake(qx, qy)]];
    }
...
CGPoint CalcStartUnitVector(NSArray *points, int count) {
    CGPoint pointAtIndex = [[points objectAtIndex:count] pointValue];
    CGPoint firstPoint = [[points objectAtIndex:0] pointValue];
